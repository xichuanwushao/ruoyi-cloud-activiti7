  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="leaveList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button v-if="2==scope.row.state" size="small" link icon="el-icon-edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['workflow:leave:edit']">修改
          </el-button>
          ...
          <!--          >销假-->
          <!--          </el-button>-->
          <el-button size="small" link icon="el-icon-edit" @click="historyFory(scope.row)"
            v-hasPermi="['workflow:leave:edit']">审批详情
          </el-button>

          <el-button v-if="0==scope.row.state" size="small" link icon="el-icon-edit"
    ...
    <el-dialog v-model="modelVisible" title="进度查询" width="1080px" append-to-body>
...
export default {
  ...
  created() {
    this.getList()
    getDicts('activiti_leave_type').then(response => {
      this.typeOptions = response.data
    })
    getDicts('activiti_flow_type').then(response => {
  ...
  methods: {
    ...
    async handleAdd() {
      ...
      console.log(user)
      this.createName = user.userName;
      if (user.userName != "admin") {
        this.reset()
        this.open = true
        this.title = '添加请假'
      } else {
        ElMessageBox.alert('管理员不能创建流程', '提示', {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有请假数据项?', '警告', {
        ...
        that.download("/danny/workflow/leave/export", {
          ...queryParams.value,
